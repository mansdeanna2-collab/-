...
1. **后端 API 服务未启动**
   - 应用需要连接后端 API 获取视频数据
   - 确保 API 服务已启动并可访问
   - 检查 `api/index.js` 中的 API 地址配置

2. **网络连接问题**
   - 确保设备能够访问配置的 API 服务器
   - 在模拟器中运行时，使用 `10.0.2.2` 替代 `localhost`
   - 检查防火墙设置是否阻止了请求

3. **等待加载状态**
   - 应用启动时会显示加载动画（🎬 图标 + 旋转加载器）
   - 如果网络慢，加载可能需要一些时间
   - 加载失败时会显示重试按钮

4. **静态资源缺失**
   - 确保 `static/tabbar/` 目录下有 `home.png` 和 `home-active.png`
   - 确保 `static/icons/` 和 `static/launch/` 目录下有对应图片文件
